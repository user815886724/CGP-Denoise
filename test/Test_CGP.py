...
def estimate(f, noise_data_numpy):
    ...
    mv_result = math_util.MV_Numpy(numpy_result)
    return mv_result

# 旧版本的训练测试方法
if __name__ == "__main__":
    model = cgp_util.load_model('model/best_model.pkl')
    first_model = model[0]
    ...
    f1 = first_model.to_numpy()
    f2 = second_model.to_numpy()
    # 每个方程进行预估噪声结果
    result1 = estimate(f1,noise_data_numpy)
    result2 = estimate(f2, noise_data_numpy)
    # 用或计算合并两个方程式的结果
    logic_result = np.logical_or(result1, result2).astype(np.int)
    for i, vote in enumerate(logic_result):
        if vote == 1:
